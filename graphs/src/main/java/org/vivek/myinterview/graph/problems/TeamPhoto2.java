...
public class TeamPhoto2 {
  ...
  public static class GraphVertex {
    public List<GraphVertex> edges = new ArrayList<GraphVertex>();
  ...
  private static Deque<GraphVertex> buildTopologicalOrdering(
      List<GraphVertex> G) {
    Deque<GraphVertex> orderedVertices = new LinkedList<GraphVertex>();
    for (GraphVertex g : G) {
      if (!g.visited) {
  ...
  public static void main(String[] args) {
    List<GraphVertex> G = new ArrayList<GraphVertex>(3);
    for (int i = 0; i < 3; i++) {
